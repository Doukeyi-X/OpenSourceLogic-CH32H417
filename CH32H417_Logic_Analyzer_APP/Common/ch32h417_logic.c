...
void Set_Logic_Para_Fun(uint8_t logic_bit, uint8_t logic_sys)
{
    ...
    case 0x0f:    // 137.5/2 = 68.75
        ...
        uhsif_div = RCC_UHSIFDIV_DIV4;
        break;
    /* Lower rates: PLL 400M (MUL16) + larger UHSIF divider, min = DIV64 */
    case 0x10:    // 400/8  = 50M
        rcc_pll = RCC_PLLMUL16;
        uhsif_div = RCC_UHSIFDIV_DIV8;
        break;
    case 0x11:    // 400/10 = 40M
        rcc_pll = RCC_PLLMUL16;
        uhsif_div = RCC_UHSIFDIV_DIV10;
        break;
    case 0x12:    // 400/16 = 25M
        rcc_pll = RCC_PLLMUL16;
        uhsif_div = RCC_UHSIFDIV_DIV16;
        break;
    case 0x13:    // 400/20 = 20M
        rcc_pll = RCC_PLLMUL16;
        uhsif_div = RCC_UHSIFDIV_DIV20;
        break;
    case 0x14:    // 400/32 = 12.5M
        rcc_pll = RCC_PLLMUL16;
        uhsif_div = RCC_UHSIFDIV_DIV32;
        break;
    case 0x15:    // 400/40 = 10M
        rcc_pll = RCC_PLLMUL16;
        uhsif_div = RCC_UHSIFDIV_DIV40;
        break;
    case 0x16:    // 400/50 = 8M
        rcc_pll = RCC_PLLMUL16;
        uhsif_div = RCC_UHSIFDIV_DIV50;
        break;
    case 0x17:    // 400/64 = 6.25M  (hardware minimum: 64x divider)
        rcc_pll = RCC_PLLMUL16;
        uhsif_div = RCC_UHSIFDIV_DIV64;
        break;
...
void HSADC_Function_Init(void)
{
    ...
    if(logic_adc_info.adc_channel == ADC_CHANNEL0)
    {
        HSADC_ChannelConfig(HSADC_Channel_4);
    }
    else if(logic_adc_info.adc_channel == ADC_CHANNEL1)
    {
        HSADC_ChannelConfig(HSADC_Channel_5);
    }
...
void LOGIC_Para_Init(void)
{
    ...
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3 | GPIO_Pin_4;      //usb link && usb trans
...
void TIM1_INT_Init( u16 arr, u16 psc)
{
    ...
    /* RCR=0: update every overflow (was 10, made LED ~11x slower) */
 ...
 * @fn      Led_Time_Init
 *
 * @brief   Acquisition LED blink timer (PB4 USB_LINK).
 *          Target: ~4 Hz full blink (on/off each ~125 ms) — lively but
 *          comfortable while capturing. Toggle each TIM1 update IRQ.
 *          f_irq ≈ TIM1_CLK / ((psc+1)*(arr+1));
 *          with TIM1_CLK≈200 MHz, psc=399, arr=62499 → ≈8 Hz IRQ → ≈4 Hz blink.
 *
 * @return  none
 */
void Led_Time_Init( void )
{
    /* was (10000, 400) + RCR=10 → ~1–2 Hz blink, felt too slow */
    TIM1_INT_Init( 62499, 399 );
    TIM_Cmd( TIM1, ENABLE );
}